...
public class CompetitionUtils {
    private static final String PATH_TO_FILE = "src\\by\\itclass\\resourses\\animals.txt";

    public static void parseFile(List<Cat> cats, List<Dog> docs, Map<String, String> errors) {
        try (Scanner sc = new Scanner(new FileReader(PATH_TO_FILE))) {
            while (sc.hasNextLine()) {
                // System.out.println(sc.nextLine());
                fillingCollections(sc.nextLine(), cats, docs, errors);
            }
    ...
    private static void fillingCollections(String textLine, List<Cat> cats, List<Dog> dogs, Map<String, String> errors) {
        try {
            ...
            processExceptions(errors, e);
        }
    }

    private static void processExceptions(Map<String, String> errors, CompetitionException e) {
       ...
       if(!str.isEmpty()){
            errors.put(str, String.format("Error in string %s - %s%n", e.getErrorLine(), e.getCause().getMessage()));
        }
    }
    ...
    public  static <T extends Animal> List<T> filterAnimals(List<T> animals, boolean isYong){
        return  animals.stream()
                .filter(isYong ? YOUNG_PREDICATE : OLD_PREDICATE)
                .collect(Collectors.toList());
    }
